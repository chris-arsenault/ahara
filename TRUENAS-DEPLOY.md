...
Use IAM Roles Anywhere for TrueNAS workloads that need AWS API access, such as S3 sync or Lambda invocation. The certificate authority runs on the trust appliance on the LAN; AWS is configured to trust it and holds no key of its own. Each app owns its per-use-case IAM role and runtime policy.

Private keys are generated by the service on first boot and stay on the service's persistent volume. The deploy workflow injects no secret at all — only the workload's identity, its role ARN, and public Roles Anywhere discovery values.

The trust appliance issues to workload ids declared in its own site policy and refuses anything else, so a container whose id is declared starts unattended on its first deploy. From then on it renews by presenting the certificate it already holds. Nothing expires on a deploy cadence, and no credential is reissued through CI.
...
The shared deploy workflow sets these Compose environment variables for the workload:
...
the project to create the listener rules, certificate, and DNS.
