...
A container's first request is held until an operator approves it in the trust appliance's portal, which binds the approval to that request's key rather than to the name it claims. From then on the container renews by presenting the certificate it already holds, so nothing expires on a deploy cadence and no credential is reissued through CI.
