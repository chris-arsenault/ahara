...
TrueNAS services use IAM Roles Anywhere, not GitHub OIDC and not static access keys. The platform owns the private CA, trust anchor, shared Roles Anywhere profile, entry role, and certificate enrollment endpoint. Each app owns one IAM role per runtime use case with a least-privilege policy.

Private keys are generated by the container on first boot and stay on persistent storage. The deploy workflow injects only a one-hour enrollment token plus public Roles Anywhere discovery values. After enrollment, the service uses normal AWS SDK credential resolution through `credential_process`.
...
In the app repo Terraform, add one `truenas-roles-anywhere-workload` module per distinct runtime permission set. The module name is arbitrary, but `prefix` must match `platform.yml` and `name` must match the `truenas_roles_anywhere` key in `platform.yml`.
...
The module creates the app-owned role, restricts trust to the matching certificate SAN URI (`spiffe://ahara/truenas/<prefix>/<name>`), and writes the role ARN to SSM where the shared deploy workflow can discover it. Keep these policies per use case; do not reuse one high-privilege TrueNAS role across unrelated services.
...
Keys under `truenas_roles_anywhere` must be lowercase DNS-label style names. `env_prefix` must be an uppercase shell identifier. During the TrueNAS deploy, the shared workflow creates a short-lived enrollment token and injects these Compose environment variables:
...
Do not add these values to `secret-paths.yml`; they are generated and injected by the shared workflow.
...
If the certificate is still valid, the service can restart without a fresh enrollment token. If the certificate is missing or near expiry, the next deploy must supply a new token, which the shared workflow does automatically on `main`.
